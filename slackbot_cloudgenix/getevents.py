...
from .helpers import table_output
...
def run(cgx_session, eventcodes, numhours, id2n, stime=None, etime=None):
    ...
    ############################################################################
    # Retrieve all events in dataframe
    ############################################################################

    csvdata = pd.DataFrame(
        columns=["time", "time_ms", "time_sms", "code", "id", "severity", "type", "correlation_id", "site", "element",
                 "entity_ref", "entity_ref text", "info", "info text", "cleared", "acknowledged",
